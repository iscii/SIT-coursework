...
(define (get-vertices G)
	;bruh i forgor that i could use map here rip woulda made it much easier but oh well
	(define (mash G)
		(cond
			((null? G) '())
			(else (append (car G) (mash (cdr G))))
		)
	)
	(usort (mash G))
)
...
;; 4 PTS
(define (degree G v)
	;; count number of times a list with v appears; +2 if reflexive
	(define (counter G v n)
		(cond 
			((null? G) n)
			((and (= (car (car G)) v) (= (car (cdr (car G))) v)) (counter (cdr G) v (+ n 2)))
			((and (= (car (car G)) v) (not (= (car (cdr (car G))) v))) (counter (cdr G) v (+ n 1)))
			((and (not (= (car (car G)) v)) (= (car (cdr (car G))) v)) (counter (cdr G) v (+ n 1)))
			(else (counter (cdr G) v n))
		)
	)
	(counter G v 0)
)
...
;; 3 PTS
(define (all-even? G)
	;if iseven? false for one vertices in G, false  otherwise true
	(define (iterator G v)
		(cond
			((null? v) #t)
			((even? (degree G (car v))) (iterator G (cdr v)))
			(else #f)
		)
	)
	(iterator G (get-vertices G))
)
...
;; 4 PTS
(define (adjacent G v)
	;; append to list if item has v in it; usort at end
	(define (finder G v)
		(cond
			((null? G) '())
			((and (= (car (car G)) v) (= (car (cdr (car G))) v)) (append (list v) (finder (cdr G) v)))
			((and (= (car (car G)) v) (not (= (car (cdr (car G))) v))) (append (list (car (cdr (car G)))) (finder (cdr G) v))) ;append the 'not' value
			((and (not (= (car (car G)) v)) (= (car (cdr (car G))) v)) (append (list (car (car G))) (finder (cdr G) v))) ;append the 'not' value
			(else (finder (cdr G) v))
		)
	)
	(usort (finder G v))
...
;; 4 PTS
(define (connected? G)
  	;call dfs on all vertices. if dfs is empty return false
	(define (checktrue l)
		(cond 
			((null? l) #t)
			((car l) (checktrue (cdr l)))
			(else #f)
		)
	)
	(checktrue (map (lambda (x) (equal? (usort (dfs G x)) (get-vertices G))) (get-vertices G)))
)
