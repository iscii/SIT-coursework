...
(define (indegree E v)
	(define (count L v n)
		(cond
		((null? L) n)
		((equal? (cadar L) v) (count (cdr L) v (+ n 1)))
		(else (count (cdr L) v n))
		)
	)
	(count E v 0)
)
...
(define (find-top G)
	(cond
		((null? (car G)) (topsort-error))
		((equal? (indegree (get-edges G) (caar G)) 0) (caar G))
		(else (find-top (cons (cdar G) (cdr G))))
	)
)
...
(define (topsort G)
	(define (halp G L)
		(cond
			((null? (get-vertices G)) L)
			(else (halp (pop-vertex G (find-top G)) (append L (list (find-top G)))))
		)
	)
	(halp G '())
)