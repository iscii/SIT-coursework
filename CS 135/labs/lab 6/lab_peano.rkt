...
#| In this lab, we'll implement Peano arithmetic in Racket.
 ...
 | Zero is the first natural number, followed by i
...
(define (p* a b)
  ;5*3 = 5+5+5
  (define (adder a b c) ;c -> current adder value
    (cond 
      ((pz? b) c)
      (else (adder a (pred b) (p+ c a)))
    )
  )
  (adder a b z)
)
...
(define (p^ a b)
  ;5^3 = 5*5*5
  ;(succ z) to get 1
  (define (multiplier a b c) ;c -> current multiplier value
    (cond 
      ((pz? b) c)
      (else (multiplier a (pred b) (p* c a)))
    )
  )
  (multiplier a b (succ z))
)
...
;; 2 PTS
(define (p= a b)
  ;should be working but whenever the function should return #f it returns nothing, gotta fix
  (cond
    ((and (pz? b) (pz? a)) #t)
    ((pz? b) #f)
    ((pz? a) #f)
...
;; 2 PTS
(define (p% a b)
  (define (subtractor a b c) ;c -> current subtractor value
    (cond 
      ((pz? b) c)
      ((p> b c) c)
      (else (subtractor a b (p- c b)))
    )
  )
  (if 
    (pz? b) (error-modulo) 
...
(define (p-prime? p)
  ;if p=0 or 1, #f
  ;for i starting from 2, i < p/2, i++, if n%i = 0, #f. otherwise #t
  (define (divider p c) ;c is a counter
    (cond
      ((and (p= p c)) #t)
      ((pz? (p% p c)) #f)
      (else (divider p (succ c)))
    )
  )
  (cond
    ((pz? p) #f)
    ((pz? (p- p (succ z))) #f)
    (else (divider p (succ (succ z))))
  )
)