...
;; 4 PTS
(define (divides? a b)
  	(or (= b 0) (and (not (= a 0)) (= (modulo b a) 0)))
)
...
(define (wj-solvable? cA cB g)
	(and (or (>= cA g) (>= cB g)) (divides? (gcd cA cB) g))
)
...
(define (waterjug cA cB g)
	(define (theactualwaterjug fA fB cA cB g)
		(cond 
			((= fB g) (list (list fA fB)))
			((= fB cB) (cons (list fA fB) (theactualwaterjug fA 0 cA cB g)))
			((= fA 0) (cons (list fA fB) (theactualwaterjug cA fB cA cB g)))
			((> (+ fA fB) cB) (cons (list fA fB) (theactualwaterjug (- fA (- cB fB)) cB cA cB g)))
			((<= (+ fA fB) cB) (cons (list fA fB) (theactualwaterjug 0 (+ fA fB) cA cB g)))
			(else "sus amogus")
		)
	)
	(theactualwaterjug 0 0 cA cB g)
)
