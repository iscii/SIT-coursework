...
(define (transitive-closure R)
	;(compose R (compose R (compose R))) (union each step) a count of (cardinality R) times.
	(define (h R curR n)
		(cond 
			((= n (cardinality R)) curR)
			(else (union curR (h R (compose R curR) (+ n 1))))
		)
	)
	(h R R 0)
)
...
;; 3 PTS
(define (transitive? R)
	;for all sets in the relation, each set must be transitive to all other sets, which is represented by transaitive-closure
...
(define (equivalence? n R)
	(and (and (transitive? R) (symmetric? R)) (reflexive? n R))
)
...
(define (involutive? n R)
	;it's bijective and symmetric
	;we have injective and symmetric
	;R is surjective if the converse of R is also injective
	(and (and (and (injective? R) (injective? (converse R))) (symmetric? R)) (set-equal? (map car (id n)) (map car R)))
)
