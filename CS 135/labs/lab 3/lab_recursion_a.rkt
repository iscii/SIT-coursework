...
; instructed to assume test cases input positive integers only
(define (sum-pow p n)
	(cond
		((= n 0) 0)
		(else (+ (expt n p) (sum-pow p (- n 1))))
	)
)
...
;assuming L is not empty and 0 <= n < L.length
(define (nth n L)
	(cond
		((= n 0) (car L))
		(else (nth (- n 1) (cdr L)))
